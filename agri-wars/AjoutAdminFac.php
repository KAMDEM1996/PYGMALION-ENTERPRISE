  ...
  @$erreur="";
   ...
   if(isset($Valider)){
      ...
      elseif($mdp!=$repass) $erreur="Mots de passe non identiques!";
         ...
         include("conn.php");
         $sel=$bd->prepare("SELECT idAd FROM admin WHERE mdp=? LIMIT 1");
         $sel->execute(array($mdp));
         $tab=$sel->fetchAll();
         if(count($tab)>0)
            $erreur="mots de pass existe déjà!";
         else{


        $fp=fopen("fichierA.txt","a+");
        fputs($fp,$NomAd.":".$PrenomAd.":".md5($mdp)."\r\n"); 

            $ins=$bd->prepare("INSERT INTO admin(NomAd,PrenomAd,mdp) VALUES(?,?,?)");
            if($ins->execute(array($NomAd,$PrenomAd,md5($mdp))))
    ...
    <a href="index.php" class="logo d-flex align-items-center scrollto me-auto me-lg-0">
      ...
      <nav id="navbar" class="navbar">
        <ul>
          ...
          <li><a class="nav-link scrollto" href="#contact"><div style=" font-weight: bold; color: Black;" class="icon"><i class="bi bi-person-badge-fill"style="font-size: 50px;"></i></div></a></li>
        </ul>
...
<div class="erreur"><?php echo $erreur ?></div>
    ...
    <p><div class="text-left mt-auto"><P><input type="submit" name="Valider" value="Valider" class="box-button" /></P></div>
