...
include_once('conn.php');
      ...
      if(($_SESSION["autoriser"]!="oui") AND ($_SESSION["identifi"]=$idAd)){
...
@$image = $_POST['Nom'].'.'.$extension;

@$cheminEtNomDefinitif = 'encyclo/'.$image;
...
@$cheminEtNomTemporaires = $_FILES['doc']['tmp_name'];

@$extensions = substr (strrchr ($_FILES['doc']['name'], "."), 1);

@$docu = $_POST['Nom'].'.'.$extensions;

@$cheminEtNomDefinitifs = 'encyclo/'.$docu;

@$moveIsOkq = move_uploaded_file($cheminEtNomTemporaires, $cheminEtNomDefinitifs);

if($moveIsOkq) {
  
  $messages = "Le fichier a été correctement uploader dans".$cheminEtNomDefinitifs;

}else{
  
  $messages = "Suite à une erreur, le fichier n'a pas uploader dans ".$cheminEtNomDefinitifs;
}
...
if(isset($_POST['Valider'])){
  ...
  $sqlq = "INSERT INTO encyclopedie(id, Nom, descriptions, Fichiers, doc) Values( '$id','$Nom','$descriptions', '$cheminEtNomDefinitif','$cheminEtNomDefinitifs' ) ";

  $resultq=mysqli_query($con,$sqlq);
      
  if($resultq){
    echo "ajout reussi!";
    header('location: encycloAj.php');
        }else{
...
<!-- ======= Section insertion d'image dans le caroussel ======= -->
    <p><section id="onfocus" class="onfocus">
<form method="POST" enctype="multipart/form-data">


<p><div class="form-group">
<p><label >Nom/Titre de l'Article</label></p>
<input type="text" class="form-control"  id="Nom" name="Nom" aria-describedby="nameHelp" placeholder="Nom du Fichier à enrégistrer!">
<small id="nameHelp" class="form-text text-muted"> nom fichier.</small>
</div></p>

<p><div class="form-group">
<p><label >Description de l'Article</label></p>
<input type="text" class="form-control"  id="descriptions" name="descriptions" aria-describedby="nameHelp" placeholder="description du Fichier à enrégistrer!">
<small id="nameHelp" class="form-text text-muted"> description article.</small>
</div></p>

<p><div class="form-button">
<p><label >Image Article</label></p>
<p><input type="file" name="Fichiers" id="Fichiers" value="<?php echo $cheminEtNomDefinitif?>">
<small id="nameHelp" class="form-text text-muted"> image article.</small></p>
</div></p>

<p><div class="form-button">
<p><label >Document à Télécharger de l'Article</label></p>
<p><input type="file" name="doc" id="doc" value="<?php echo $cheminEtNomDefinitifs?>">
<small id="nameHelp" class="form-text text-muted"> doc article.</small></p>
</div></p>
...
<button type="Submit" name="Valider" class="btn btn-primary">Valider</button>
    ...
    </section><!-- End On Focus Section insertion d'image dans le caroussel --></p>

<!-- ======= Section ======= -->
<section  class="container-fluid p-0">

<p><div><span><h5 href="#" class="btn btn-secondary"> <samp> Section Encyclopédie .</samp> </h5></span></div></p>  
...
$acte++; ?>
  ...
  <div class="card  <?php if($acte==1){echo 'active';} ?>" >
    <img class="card-img-top" value="<?php echo $keys;?>" src="<?php echo $values["Fichiers"]; ?>" alt="Card image cap" style="width: 18rem; border-top-width: 5px;">
    <div class="card-body" style="color: black; font:bold; font-weight: bold; ">
      <h5 class="card-title"><?php echo $values["Nom"];?></h5>  <h5 class="card-title"><?php echo $values["descriptions"];?></h5>
     <h5 class="card-title">#<?php echo $values["id"];?> <samp></samp> <small class="text-muted"><?php 
       echo '<button class="btn btn-danger"><a href="SuppressionEncyclo.php? Supid='.$values["id"].' "class="text-light"><span> Supprimer Encyclopédie </span></a></button>';  }  ?></small></h5> 
     </h5><br> 
